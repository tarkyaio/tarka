...
def index_investigation_run(
    *,
    ...
    """
    Persist an investigation run index row in Postgres.
    ...
    inp = CaseizeInput(
        ...
        container=_safe_str(investigation.target.container),
    ...
    with _connect(dsn) as conn:
        with conn.transaction():
            ...
            # Update case summary for faster browsing
            conn.execute(
                """
                UPDATE cases
                SET updated_at = now(),
    ...
    return True, "indexed", IndexResult(case_id=case_id, run_id=run_id, case_match_reason=reason)
