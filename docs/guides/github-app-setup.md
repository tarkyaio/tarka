...
- The **`-git` image variant** — the default image does not include `git`. See [Image tag variants](helm-chart.md#image-tag-variants).
...
> image does not include `git` — use the `-git` tag variant (e.g. `ghcr.io/tarkyaio/tarka:0.3.2-git`).
...
3. Git not available in runtime image — using the default image instead of the `-git` variant

**Solutions**:
- Ensure you are using the `-git` image tag (e.g. `ghcr.io/tarkyaio/tarka:0.3.2-git`); the default image does not ship `git`
