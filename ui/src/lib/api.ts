  ...
  const request = React.useCallback(
    async function request<T>(path: string, init?: RequestInit): Promise<T> {
      ...
      if (mockMode) {
        ...
        if (p === "/api/v1/exec/overview") {
          const now = Date.now();
          const day = (daysAgo: number) =>
            new Date(now - daysAgo * 86400000).toISOString().slice(0, 10);
          return {
            risk: {
              active_count: 14,
              active_high_impact_count: 3,
              stale_investigation_count: 2,
              oldest_active_created_at: new Date(now - 18 * 3600000).toISOString(),
              critical_this_month: 5,
              total_this_month: 31,
              top_active: [
                {
                  incident_id: "case_1",
                  ...
                  one_liner: "Payment gateway p99 latency > 2s",
                  team: "Payments",
                ...
                },
                {
                  ...
                  one_liner: "Auth service crash-looping, OOM suspected",
                  team: "Identity",
                ...
                },
                {
                  ...
                  one_liner: "Nightly reconciliation job failed twice",
                  team: "Platform",
                  service: "reconciler",
                  family: "job_failure",
                  impact_score: 61,
                ...
                },
                {
                  ...
                  one_liner: "Postgres primary disk at 87%",
                  team: "Data",
                ...
                },
                {
                  ...
                  one_liner: "metrics-exporter scrape failing intermittently",
                  team: "Observability",
            ...
            trends: {
              daily: Array.from({ length: 30 }, (_, i) => ({
                day: day(29 - i),
                incidents_created: Math.max(
                  0,
                  Math.round(3 + Math.sin(i / 3) * 2 + Math.random() * 2)
                ),
                impact_median: Math.round(40 + Math.sin(i / 4) * 20 + Math.random() * 10),
              })),
              mttr_weekly: Array.from({ length: 8 }, (_, i) => ({
                week: day((7 - i) * 7),
                mttr_hours_median: parseFloat(
                  (1.5 + Math.sin(i / 2) * 0.8 + Math.random() * 0.5).toFixed(1)
                ),
                resolved_count: Math.round(8 + Math.random() * 6),
              })),
            },
            focus: {
              top_teams: [
                { team: "Payments", active_count: 4, high_impact_count: 2, total_impact: 312 },
                { team: "Identity", active_count: 3, high_impact_count: 1, total_impact: 198 },
                { team: "Platform", active_count: 4, high_impact_count: 0, total_impact: 156 },
                { team: "Data", active_count: 2, high_impact_count: 0, total_impact: 98 },
                { team: "Observability", active_count: 1, high_impact_count: 0, total_impact: 42 },
              ],
              top_drivers: [
                { driver: "oom_kill", active_count: 5, high_impact_count: 2, total_impact: 280 },
                {
                  driver: "http_latency_spike",
                  active_count: 4,
                  high_impact_count: 1,
                  total_impact: 240,
                },
                {
                  driver: "config_change",
                  active_count: 3,
                  high_impact_count: 0,
                  total_impact: 145,
                },
                {
                  driver: "disk_pressure",
                  active_count: 2,
                  high_impact_count: 0,
                  total_impact: 98,
                },
              ...
              top_services: [
                {
                  service: "payments-svc",
                  incident_count: 8,
                  unique_alert_types: 4,
                  median_impact: 88,
                  change_correlated_count: 3,
                },
                {
                  service: "auth-service",
                  incident_count: 6,
            ...
            recurrence: {
              rate: 0.24,
              top: [
                { incident_key: "HttpLatencyHigh:payments-svc:prod", count: 7 },
                { incident_key: "PodCrashLooping:auth-service:prod", count: 4 },
                { incident_key: "DiskUsageHigh:postgres-primary:prod", count: 3 },
              ],
            },
            ai: {
              ttfa_median_seconds: 47,
              ttfa_p90_seconds: 112,
              confidence_ge_70_pct: 68,
              gaps_pct: { missing_one_liner: 8, missing_team: 14, missing_family: 11 },
            },
            signal: {
              total_runs: 247,
              actionable: 89,
              ...
              unclassified: 15,
              actionable_pct: 36,
              change_correlated_count: 8,
              change_correlated_pct: 57,
            },
            savings: {
              total_runs: 247,
              high_conf_runs: 168,
              ...
              deflected_runs: 143,
              hours_saved: 74.3,
              cost_saved_usd: 11145,
              triage_minutes_assumed: 20,
              hourly_rate_usd_assumed: 150,
            },
            cost: {
              total_usd: 5.87,
              avg_per_run_usd: 0.0238,
              total_runs: 247,
              daily: Array.from({ length: 30 }, (_, i) => ({
                day: day(29 - i),
                cost_usd: parseFloat(
                  (0.08 + Math.sin(i / 4) * 0.05 + Math.random() * 0.04).toFixed(4)
                ),
              })),
            },
          } as unknown as T;
